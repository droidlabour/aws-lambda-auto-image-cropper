# coding=utf-8
from deps import util
from deps.transform import four_point_transform
import cv2
...
def scan(im_path, show=True):
    ...
    cv2.imwrite('/tmp/edged.jpg', edged)
    ...
    if screenCnt.__len__() != 0:
        if show:
            ...
            cv2.imwrite('/tmp/outlined.jpg', im)
        warped = four_point_transform(orig, screenCnt.reshape(4, 2) * scale)
    else:
        warped = orig

    cv2.imwrite('/tmp/deskewed.jpg', warped)
# scan()
