...
from deps.crop import crop
from deps.scan import scan
...
def recognize(imgname, output, desired,
    ...
    im = cv2.imread('/tmp/deskewed.jpg')
    im = cv2.dilate(im, np.ones((2, 2)))
    newimgname = '/tmp/no_noise.jpg'
    cv2.imwrite(newimgname, im)
    crop(newimgname, '/tmp/scan_res.jpg', show_intermediate_results)
    recognized_text = pytesseract.image_to_string(Image.open('/tmp/scan_res.jpg'), config="config")
